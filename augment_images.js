...
async function run() {
    let images = await readImagesDirectory(args.images_dir).catch((e) => { console.log('readImagesDirectory:', e) });
    ...
    for (const item of images) {
        const files = await Promise.all(
            item.images.map(async name => {
                return {
                    image: await sharp(name).catch((e) => { console.log('sharp:', e) }),
        ...
        if (!labels_to_skip.includes(item.label.toLowerCase())) {
            if (args.flip_images) {
                ...
                const newNames = await augment_flip(files, true, true).catch((e) => { console.log('augment_flip:', e) });
            }
            if (args.adjust_brightness) {
                console.log(`Adjusting Brightness: ${item.label}`);
                const newNames = await augment_brightness(files).catch((e) => { console.log('augment_brightness:', e) });
            }
        }
    }
}
...
async function augment_brightness(files, brightnessDelta = 0.2) {
    ...
    for (const item of files) {
        ...
        let writePromises = [];
        await fse.ensureDir(baseDir).catch((e) => { console.log('ensureDir:', e) });
...
async function augment_flip(files, flip_x, flip_y) {
    ...
    for (const item of files) {
        ...
        let writePromises = [];
        if (flip_x || flip_y) await fse.ensureDir(baseDir).catch((e) => { console.log('ensureDir:', e) });
...
async function readImagesDirectory(imagesDirectory) {
    let directories = args.ignore_subdirectories
        ? [""]
        : await getDirectories(imagesDirectory).catch((e) => { console.log('getDirectories:', e) });
