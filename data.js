...
class Data {
    ...
    async loadLabelsAndImages(imagesDirectory) {
        this.labelsAndImages = await readImagesDirectory(imagesDirectory).catch((e) => { console.log('readImagesDirectory:', e) });
    }
    ...
    async loadTrainingData(model) {
        ...
        console.log('embeddingsShape', embeddingsShape)
        const embeddingsFlatSize = tf.util.sizeFromShape(embeddingsShape);
        console.log('embeddingsFlatSize', embeddingsFlatSize)
        ...
        for (const element of this.labelsAndImages) {
            ...
            for (const image of element.images) {
                let t = await fileToTensor(image).catch((e) => { console.log('fileToTensor:', e) });
                // console.log('t', t)
                tf.tidy(() => {
